...
class CategoryManager:
    ...
    def __init__(self, config_file: str = "categories_config.json"):
        ...
        # Integrar RuleEngine para reglas avanzadas
    ...
    def categorize_file(self, file_path: Path) -> str:
        """Categoriza un archivo usando reglas personalizadas, rule engine y extensión"""
        # 1. Primero verificar RuleEngine (reglas avanzadas)
        rule_category = self.rule_engine.categorize_file(file_path)
        if rule_category:
            return rule_category
        
        # 2. Luego verificar reglas personalizadas legacy (ordenadas por prioridad)
        for rule in sorted(self.custom_rules, key=lambda r: r.priority, reverse=True):
            if rule.matches(file_path):
                return rule.category

        # 3. Si no hay reglas, usar categorización por extensión
