...
def _log(msg: str) -> None:
    """Log seguro para evitar bloqueos en PyInstaller durante inicialización."""
    try:
        print(msg)
...
class _SignalEmitter(QObject):
    ...
    def __init__(self, parent=None):
        ...
        _log("[SignalEmitter] Creando QObject para señales...")
        try:
            super().__init__(parent)
            _log("[SignalEmitter] QObject inicializado correctamente")
        except Exception as e:
            _log(f"[SignalEmitter] ERROR: {e}")
            # Intentar sin parent
            try:
                super().__init__(None)
                _log("[SignalEmitter] QObject inicializado sin parent")
            except Exception as e2:
                _log(f"[SignalEmitter] ERROR CRÍTICO: {e2}")
                raise
...
class ApplicationState:
    ...
    def __new__(cls):
        ...
        if cls._instance is None:
            with cls._lock:
                if cls._instance is None:
                    _log("[AppState] Creando nueva instancia en __new__...")
    ...
    def __init__(self):
        ...
        _log("[AppState] Iniciando __init__ de ApplicationState...")
        
        # ⚠️ CRÍTICO: NO heredar de QObject directamente - usar uno interno
        # Esto evita el bloqueo en PyInstaller
        _log("[AppState] QObject interno se creará bajo demanda cuando se necesite...")
        
        _log("[AppState] Configurando atributos iniciales...")
        ...
        _log("[AppState] Atributos y locks inicializados")
        
        # === INICIALIZACIÓN ===
        _log("[AppState] Llamando _initialize_components()...")
        self._initialize_components()
        _log("[AppState] _initialize_components() completado")
        
        self._initialized = True
        _log("[AppState] Flag _initialized = True")
        
        # Emitir evento de inicialización (diferido - después de que UI esté lista)
        _log("[AppState] Evento de inicialización se emitirá cuando UI esté lista")
    ...
    def _get_signal_emitter(self) -> Optional[_SignalEmitter]:
        ...
        if self._signal_emitter is None:
            _log("[AppState] Creando QObject interno para señales (primera vez)...")
            try:
                # Intentar obtener QApplication como parent
                from PyQt6.QtWidgets import QApplication
                app = QApplication.instance()
                parent = app if app else None
                _log(f"[AppState] QApplication parent: {parent is not None}")
                
                self._signal_emitter = _SignalEmitter(parent)
                _log("[AppState] QObject interno creado exitosamente")
            except Exception as e:
                _log(f"[AppState] ERROR creando QObject interno: {e}")
                import traceback
                _log(f"[AppState] Traceback: {traceback.format_exc()}")
                # Crear sin parent como último recurso
                try:
                    self._signal_emitter = _SignalEmitter(None)
                    _log("[AppState] QObject interno creado sin parent")
                except Exception as e2:
                    _log(f"[AppState] ERROR CRÍTICO: No se pudo crear QObject interno: {e2}")
    ...
    def _initialize_components(self):
        ...
        """
        try:
            _log("[AppState] Inicializando componentes (modo super lazy)...")
            ...
            _log("[AppState] Componentes en modo lazy (se crearán bajo demanda)")
            
        except Exception as e:
            # ⚠️ ÚLTIMO RECURSO: Capturar TODO para evitar que el módulo falle
            import traceback
            _log(f"[AppState] ERROR CRÍTICO en _initialize_components: {e}")
            _log(f"[AppState] Traceback completo:")
            traceback.print_exc()
    ...
    def get_app_config(self) -> Optional['AppConfig']:
        ...
        if self.app_config is None:
            with self._lock:
                if self.app_config is None:
                    try:
                        _log("[AppState] Inicializando AppConfig de forma lazy...")
                        from src.utils.app_config import AppConfig
                        ...
                        _log("[AppState] AppConfig inicializado")
                        self._emit_event(EventType.STATE_CHANGED, {
                            "component": "app_config",
                            "action": "initialized"
                        }, "ApplicationState")
                    except Exception as e:
                        _log(f"[AppState] ERROR creando AppConfig: {e}")
                        import traceback
                        traceback.print_exc()
                        self.app_config = None
    ...
    def get_category_manager(self) -> Optional['CategoryManager']:
        ...
        if self.category_manager is None:
            with self._lock:
                if self.category_manager is None:
                    try:
                        _log("[AppState] Inicializando CategoryManager de forma lazy...")
                        from .category_manager import CategoryManager
                        self.category_manager = CategoryManager()
                        _log("[AppState] CategoryManager inicializado")
                        self._emit_event(EventType.STATE_CHANGED, {
                            "component": "category_manager",
                            "action": "initialized"
                        }, "ApplicationState")
                    except Exception as e:
                        _log(f"[AppState] ERROR creando CategoryManager: {e}")
                        import traceback
                        traceback.print_exc()
                        self.category_manager = None
        ...
        if theme != self.current_theme:
            ...
            if app_config:
                try:
                    ...
                    _log(f"[AppState] Error guardando tema: {e}")
        ...
        if font_size != self.current_font_size:
            ...
            if app_config:
                try:
                    ...
                    _log(f"[AppState] Error guardando font_size: {e}")
    ...
    def _emit_event(self, event_type: EventType, data: Dict[str, Any], source: str):
        ...
        if emitter:
            try:
                ...
                _log(f"[AppState] Error emitiendo señal: {e}")
...
def get_app_state() -> ApplicationState:
    ...
    if _app_state_instance is None:
        with _app_state_lock:
            if _app_state_instance is None:
                _log("[AppState] Creando instancia global de ApplicationState...")
                try:
                    _app_state_instance = ApplicationState()
                    _log("[AppState] Instancia global creada OK")
                except Exception as e:
                    _log(f"[AppState] ERROR CRÍTICO al crear instancia global: {e}")
                    import traceback
                    traceback.print_exc()
                    raise
...
class _AppStateProxy:
    ...
    def __getattr__(self, name):
        ...
        try:
            ...
            if instance is None:
                _log(f"[AppStateProxy] ApplicationState no disponible, '{name}' = None")
                return None
            ...
            _log(f"[AppStateProxy] ERROR accediendo a '{name}': {e}")
            import traceback
            traceback.print_exc()
        ...
        if name.startswith('_'):
            ...
            try:
                ...
                if instance is not None:
                    ...
                    _log(f"[AppStateProxy] No se puede establecer '{name}': ApplicationState no disponible")
            except Exception as e:
                _log(f"[AppStateProxy] ERROR estableciendo '{name}': {e}")
                import traceback
                traceback.print_exc()
