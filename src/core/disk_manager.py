...
class DiskManager:
    ...
    def __init__(self):
        ...
        self._drive_map = None  # Inicia como None para lazy loading
        self._smart_cache = SmartCache(ttl_seconds=30)  # Cache con TTL de 30 segundos
        # FIX: Cache global SMART para evitar escaneos duplicados por disco
        self._smart_data_global_cache: Dict[str, dict] = {}
        self._smart_scan_lock = threading.Lock()  # Lock exclusivo para SMART
        ...
        if self.smartctl.is_available():
            success("smartctl disponible - Se usaran datos SMART reales lifetime")
        else:
            warn("smartctl no disponible - Se usaran datos psutil como fallback")
        
        # Eliminado todo el WMI de __init__
    ...
    def _get_smart_data(self, drive_letter: str) -> Optional[dict]:
        ...
        FIX: Usa lock global y cache global para evitar escaneos duplicados por disco."""
        if not drive_letter:
            return None
        ...
        if not self._drive_map:
            with self._smart_scan_lock:
                if not self._drive_map:
                    debug("Creando mapeo de unidades usando smartctl...")
                    self._drive_map = self._map_drives_with_smartctl_only()
        
        drive_key = (drive_letter + ":").upper()
        drive_info = self._drive_map.get(drive_key)
        if not drive_info:
            return None
        ...
        # FIX: Cache global PRIMERO (antes de intentar lock)
        cache_key = f"smart_{physical_drive_id}"
        if cache_key in self._smart_data_global_cache:
            return self._smart_data_global_cache[cache_key]
        
        # Tambien verificar caché con TTL
        cached_ttl = self._smart_cache.get(cache_key)
        if cached_ttl:
            self._smart_data_global_cache[cache_key] = cached_ttl
            return cached_ttl
        
        # FIX: Lock para evitar escaneo duplicado CONCURRENTE del mismo disco
        with self._smart_scan_lock:
            # Doble-check tras adquirir lock
            if cache_key in self._smart_data_global_cache:
                return self._smart_data_global_cache[cache_key]
            
            # Ya otro hilo esta escaneando este disco?
            scanning_key = f"scanning_{physical_drive_id}"
            if self._smart_data_global_cache.get(scanning_key):
                return None  # Otro hilo lo esta escaneando, devolver None
            
            try:
                # Marcar como escaneando para evitar duplicados
                self._smart_data_global_cache[scanning_key] = True
                ...
                # Guardar en AMBAS caches
                self._smart_cache.set(cache_key, result)
                self._smart_data_global_cache[cache_key] = result
                return result
                
            finally:
                # Limpiar flag de escaneo
                self._smart_data_global_cache.pop(scanning_key, None)
