...
class ThemeApplier:
    ...
    def apply_theme_to_widget(
        ...
        Args:
            ...
            respect_existing: Si True, preserva estilos locales con objectName/clases
        ...
        try:
            ...
            ThemeApplier._apply_to_children(
                widget,
                palette,
                css_styles,
                force_override,
    ...
    def _apply_to_children(
        ...
        try:
            for child in parent.findChildren(QWidget):
                if (
                    child.parent() == parent
                ):  # Solo hijos directos para evitar duplicados
                    try:
                        # Aplicar paleta
                        ...
                        if (
                            force_override
                            and not ThemeApplier._should_preserve_widget_style(
                                child, respect_existing
                            )
                        ):
                            child.setStyleSheet("")
                            ...
                            if (
                                ...
                                or not ThemeApplier._should_preserve_widget_style(
                                    child, respect_existing
                                )
                            ):
                                child.setStyleSheet(css_styles)
    ...
    def apply_theme_to_dialog(
        ...
        try:
            ...
            # Aplicar tema base
            ThemeApplier.apply_theme_to_widget(
                dialog,
                theme_name,
                font_size,
            ...
            QTimer.singleShot(
                100,
                lambda: ThemeApplier._delayed_theme_apply(
                    dialog, theme_name, font_size
                ),
            )
    ...
    def _get_dialog_specific_css(colors: Dict[str, str], font_size: int) -> str:
        ...
        QPushButton:hover {{
            background-color: {colors.get("button_hover", colors.get("secondary", colors["primary"]))} !important;
        }}
        
        QPushButton:pressed {{
            background-color: {colors.get("button_pressed", colors.get("primary", "#1976d2"))} !important;
        }}
        
        QPushButton:disabled {{
            background-color: {colors.get("text_disabled", "#bdbdbd")} !important;
            color: {colors.get("text_secondary", "#757575")} !important;
        }}
    ...
    @staticmethod
    def _fix_problematic_elements(dialog: QWidget, colors: Dict[str, str]):
        """Arregla elementos específicos que no cambian correctamente"""
        try:
            # Buscar y arreglar elementos específicos
            for widget in dialog.findChildren(QWidget):
                if ThemeApplier._should_preserve_widget_style(widget, True):
                    continue
                ...
                if widget_type == "QGroupBox":
                    ...
                    widget.setStyleSheet(f"""
                        QPushButton {{
                            background-color: {colors.get("primary", "#1976d2")} !important;
                            color: white !important;
                            border: none !important;
                            border-radius: 6px !important;
                        ...
                        QPushButton:hover {{
                            background-color: {colors.get("button_hover", colors.get("secondary", "#1565c0"))} !important;
                        }}
                        QPushButton:pressed {{
                            background-color: {colors.get("button_pressed", colors.get("primary", "#0d47a1"))} !important;
                        }}
                        QPushButton:disabled {{
                            background-color: {colors.get("text_disabled", "#bdbdbd")} !important;
                            color: {colors.get("text_secondary", "#757575")} !important;
    ...
    def apply_theme_globally(theme_name: str, font_size: int = 12) -> bool:
        ...
        try:
            ...
            if app is None:
                return False
            app_qt = app if isinstance(app, QApplication) else None
            if app_qt is None:
                return False
            ...
            app_qt.setStyleSheet(css_styles)
    ...
    @staticmethod
    def _should_preserve_widget_style(widget: QWidget, respect_existing: bool) -> bool:
        """Determina si un widget ya tiene estilos específicos que deben preservarse."""
        if not respect_existing:
            return False

        try:
            current_style = widget.styleSheet() or ""
            if current_style and (widget.objectName() or "!important" in current_style):
                return True

            for dynamic_property in ("styleClass", "class"):
                value = widget.property(dynamic_property)
                if value:
                    return True
        except Exception:
            return False
